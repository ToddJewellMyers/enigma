...
AI assistance is intentionally disabled in the current Enigma terminal. The application does not read an API key, call an AI provider, or transmit terminal output.
...
7. Keep AI access inside the trusted local terminal window and a main-process IPC handler. The hosted Enigma page must not receive the API key or AI response.
