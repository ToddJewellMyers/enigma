...
type ColumnProps = { id: string; title: string; refreshVersion: number; onCardMoved: () => void };
...
function Column({ id, title, refreshVersion, onCardMoved }: ColumnProps) {
    ...
    return <div data-column-id={id} onDragOver={(event) => { event.preventDefault(); event.dataTransfer.dropEffect = "move"; setIsDragOver(true); }} onDragLeave={() => setIsDragOver(false)} onDrop={(event) => { event.preventDefault(); setIsDragOver(false); const cardId = event.dataTransfer.getData("text/plain"); if (cardId) void cardState.move(cardId, cardState.cards.length + 1); }} className={`min-w-0 rounded-xl border p-4 transition md:min-w-[300px] ${isDragOver ? "border-blue-500 bg-blue-950/40" : "border-slate-800 bg-slate-900/80"}`}>
        ...
        <NewCardForm columnTitle={title} onCreate={cardState.add} />
        {cardState.error && <p role="alert" className="mb-3 rounded bg-red-950/50 p-2 text-xs text-red-300">{cardState.error}</p>}
        <div className="flex flex-col gap-3">
            {cardState.cards.map((card, index) => <Card card={card} onDelete={() => void cardState.remove(card.id)} onSave={(changes) => cardState.update(card.id, changes)} onDropCard={(cardId) => cardState.move(cardId, index + 1)} key={card.id} />)}
        </div>
    </div>;
}
