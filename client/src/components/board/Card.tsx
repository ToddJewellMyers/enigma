...
type CardProps = { card: KanbanCard; onDelete: () => void; onSave: (update: CardUpdate) => Promise<void>; onDropCard: (cardId: string) => Promise<void>; canEdit: boolean };

function Card({ card, onDelete, onSave, onDropCard, canEdit }: CardProps) {
    ...
    return <CardDisplay card={card} onEdit={() => setIsEditing(true)} onDelete={onDelete} onDropCard={onDropCard} canEdit={canEdit} />;
}
