...
type BoardProps = { selectedWorkspace: Workspace | null };
...
function Board({ selectedWorkspace }: BoardProps) {
    ...
    return <section>
        <BoardHeader workspaceName={selectedWorkspace.name} boardName={boardState.activeBoard?.name} onCreate={boardState.addBoard} />
        {boardState.error && <p role="alert" className="mb-5 rounded-lg bg-red-950/50 p-3 text-sm text-red-300">{boardState.error}</p>}
        <BoardTabs boards={boardState.boards} activeId={boardState.activeBoardId} onSelect={(id) => void boardState.selectBoard(id)} onDelete={(board) => void boardState.removeBoard(board)} />
        <div className="grid grid-cols-1 gap-5 pb-4 md:flex md:overflow-x-auto">
            {boardState.columns.map((column) => <Column id={column.id} title={column.name} refreshVersion={cardRefreshVersion} onCardMoved={() => setCardRefreshVersion((version) => version + 1)} key={column.id} />)}
        </div>
    </section>;
}
