...
function Board({ selectedWorkspace, onOpenTeam, realtimeRevision }: BoardProps) {
    ...
    return <section>
        ...
        <div className="grid grid-cols-1 gap-5 pb-4 md:flex md:overflow-x-auto">
            {boardState.columns.map((column) => <Column id={column.id} title={column.name} refreshVersion={cardRefreshVersion + realtimeRevision} onCardMoved={() => setCardRefreshVersion((version) => version + 1)} onDelete={() => void boardState.removeColumn(column)} canEdit={selectedWorkspace.role !== "Viewer"} key={column.id} />)}
        </div>
    </section>;
}
